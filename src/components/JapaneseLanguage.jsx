    ...
    <section className="w-full rounded-xl border border-blue-800 bg-white shadow-sm overflow-hidden ">
      ...
      {japaneseCourseName.map((item, index) => (
        ...
        >
          ...
          />
          <InputField
            label={`${item.name} এর রোল/রেজিস্ট্রেশন নং`}
            type="number"
            className={item.className}
          ...
          />
          <InputField
            label={`${item.name} এর সম্ভাব্য পরীক্ষার তারিখ (পরীক্ষা না দিয়ে থাকলে)`}
            type="date"
            className={item.className}
